...
export const createProject = async (req, res) => {
  try {
    ...
    if (!workspace) {
      return res.status(404).josn({ message: "Workspace not found" });
    }
    if (
    ...
    res.josn({project:projectWithMembers, message: 'project created successfully' })
  } catch (error) {
    console.log(error);
    res.status(500).josn({ message: error.code || error.message });
  }
};
...
export const addMember = async (req, res) => {
  try {
    ...
    if(project){
        return res.status(404).json({message: "Project n ot found" });
    }
